...
`RoomResetter.Describe()` keys what it offers off those three write points rather than off room type alone. It reads `RunState.BaseRoom`, not `CurrentRoom`, because an event option that starts a fight pushes a `CombatRoom` on top while the save still replays the map point — what actually comes back is the event. Where the game has already written a pre-finished-room save over the opening state it still offers the restart, but names where the replay lands instead of claiming a fresh start: a combat room with no fight in progress (won, sitting on rewards) and an event whose map point hosted a combat room that is no longer running both replay the reward screen, and an `EventRoom` with `IsPreFinished` (Ancient, done) replays from just after it resolved. Merchants, rest sites and treasure rooms never save, so they replay from the start.
...
So the record rewinds with the game, **to the same point the save on disk describes** — which is not always room entry. The rule is one snapshot per game save, taken at the save:

- `RunSaveSnapshotPatch` prefixes `RunSaveManager.SaveRun(AbstractRoom)`, the overload every gameplay save is serialized in (map-node entry, combat victory and a finished Ancient event all reach it through `SaveManager.SaveRun`, which first awaits any save still in flight). The prefix runs in the same synchronous segment as the method's own `RunManager.ToSave`, so `RunTracker.OnRunSaveWriting` serializes `_currentRun` (through `RunStorage.Options`, so it round-trips the on-disk shape) at the exact instant the game serializes the save. It is gated on `RunTracker.GameWritesRunSave()`, which mirrors the method's own `ShouldSave && (Singleplayer || Host)` condition, so a skipped save never moves the snapshot.
- Taking it at the write rather than at `Hook.AfterRoomEntered` matters. Everything after the map-node save — `BeforeRoomEntered`, room creation, the room-entry recorders in `SignetRingStatsPatch`, relic room-entry effects, the visit itself — is replayed by the game when it loads the save. A snapshot taken after any of it would already contain what the replay does again. (An earlier build snapshotted at `AfterRoomEntered`; it survived only because the room-entry recorders it swept in are floor-idempotent.)
- **Combat victory is the one deferred capture.** `CombatManager.EndCombatInternal` calls `room.MarkPreFinished()` and *awaits* `SaveRun(room, saveProgress: false)` before it raises `CombatEnded`, and SpireLens promotes `_pendingCombat` on `CombatEnded`. So at that save `OnRunSaveWriting` drops the stale snapshot and records the save point it is owed for, and `RunTracker.OnCombatEnded` takes it right after the promotion. Rewinding to room entry instead would **erase** the won fight the game keeps — the mirror image of the double-count and just as much a defect. The window in between — `UpdateProgressAfterCombatWon`, achievements, `CombatWon` — changes no run state the save carries, and while it is open there is no snapshot rather than a wrong one. Event-spawned fights take the same path: their victory save is the same call with the stacked `CombatRoom`.
- A **finished Ancient** needs nothing special: `EventRoom.OnEventStateChanged` marks the room and fires `SaveRun(this)` after the chosen option's `RelicCmd.Obtain`, so the snapshot taken at that write carries the relic.
- A replayed pre-finished `CombatRoom` goes through `StartPreFinishedCombat`, which fires neither `AfterRoomEntered` nor `CombatEnded`, and a replay never saves (`LoadIntoLatestMapCoord` passes `saveGame: false`), so the snapshot survives any number of replays of the same save.

Each snapshot is tagged with the save point it describes, from `RunTracker.DescribeRunSavePoint`: act index, map coordinate (`CurrentMapCoord`, the last visited coord) and `TotalFloor`. The floor is what separates two saves at the same map point: the map-node save is written before `EnterMapPointInternal` appends the point's history entry, and every later save there after it, so a room-entry snapshot can never be mistaken for a pre-finished-room save or the other way round.

The snapshot is restored on both paths that replay the save:

- **Room restart.** `RunTracker.RollBackToLastRunSave` restores it, and `RoomResetter` calls it after every abort path and immediately before `CleanUp()` — the point of no return — so a corrupt-save abort leaves both the game and the record untouched. It writes via `RunStorage.SaveAsync` rather than `SaveCurrentRun`, because the latter re-stamps the still-live (post-purchase) instance numbers over the restored ones. The live identity maps are left alone on purpose: `RunStarted` re-fires moments later and `AdoptRunLocked` clears and rebuilds them from the restored record against the reloaded deck.
- **Main-menu Continue.** Save and Quit mid-shop or mid-event followed by Continue replays the room from the same save, but adoption alone only drops `_pendingCombat` — shop and event results are already committed. So both `OnRunStarted` adoption paths (the in-memory record, and the on-disk record after a game restart) first pass the record through `RewindToLastRunSaveLocked`. `RunStarted` fires from `RunManager.Launch` after `RunState.FromSerializable` has rebuilt the state from the save, so `DescribeRunSavePoint(runState)` names the save being loaded; the snapshot is used only when its tag equals that. A mismatch (a snapshot from an older build, or none) leaves the record as it was and logs why. A RoomResetter restart goes through this path too and rewinds to the same snapshot a second time, which is a no-op. `TryResumeActiveRun` (Core hot reload) does **not** rewind: the game is not replaying anything.

The snapshot is held in memory and mirrored to disk at every capture, under `user://SpireLens/run-save-snapshot/<run_id>.json` as `{ "save_point": …, "run": <RunData> }` — deliberately not under `runs/`, which is enumerated as run records. It is a cache, not part of the run record's schema. A Core hot reload or a game restart therefore keeps it: `EnsureRunSaveSnapshotLoadedLocked` reads the file back when memory is empty. A capture that fails deletes the file instead of leaving the previous snapshot behind, because an older snapshot read back no longer matches the save. With no snapshot at all, `Describe()` blocks with `no run-save snapshot` and Continue does not rewind — refusing a restart is cheap, silently inflating or erasing part of a run is not. Untagged snapshots written by earlier builds under `user://SpireLens/room-entry/` are never read.

An Ancient's arrival heal (`AncientEventModel.BeforeEventStarted`, inside `EventRoom.EnterInternal`) is not in any snapshot and not in the record at all: `CreatureCmd.Heal` only raises `Hook.AfterCurrentHpChanged` when `creature.CombatState != null`, and the player's creature is unattached from combat state outside a fight (`CombatManager.Reset` and `CombatState.RemoveCreature(unattach: true)`), so `HookAfterCurrentHpChangedPatch` never sees it.
