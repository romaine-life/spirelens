...
So the record rewinds with the game, **to the same point the save on disk describes** — which is not always room entry:

- `RunTracker.CaptureRoomEntrySnapshot` serializes `_currentRun` (through `RunStorage.Options`, so it round-trips the on-disk shape) when a room opens. The call rides `SignetRingStatsPatch`, the existing shared observer on `Hook.AfterRoomEntered`. That hook fires just after `EnterMapPointInternal` wrote the run save, so the snapshot and the save describe the same instant by construction. It is taken last in that patch, after the room-entry recorders, and skipped when `CurrentRoomCount > 1` so an event-spawned combat cannot overwrite the event's snapshot.
- `RunTracker.CaptureRewindSnapshotAtPreFinishedSave` retakes it whenever the game rewrites the save with a pre-finished room, because from then on a restart replays that save, not the room's opening. Rewinding to room entry there would **erase** what the game keeps — the won fight's promoted stats, the Ancient's relic — which is the mirror image of the double-count and just as much a defect. Both retakes are gated on `RunTracker.GameWritesRunSave()`, which mirrors `RunSaveManager.SaveRun`'s own `ShouldSave && (Singleplayer || Host)` condition so a skipped save never moves the snapshot:
  - **Combat victory.** `CombatManager.EndCombatInternal` calls `room.MarkPreFinished()` and *awaits* `SaveRun(room, saveProgress: false)` before it raises `CombatEnded`, and SpireLens promotes `_pendingCombat` on `CombatEnded`. So the retake cannot happen at the write itself; it happens in `RunTracker.OnCombatEnded` right after the promotion, keyed on `room.IsPreFinished` (a loss never marks the room). The window in between — `UpdateProgressAfterCombatWon`, achievements, `CombatWon` — changes no run state the save carries. This covers event-spawned fights too: their victory save is the same call with the stacked `CombatRoom`, which is why this path does not apply the `CurrentRoomCount > 1` skip.
  - **Ancient event finished.** `AncientEventRewindSnapshotPatch` wraps `EventRoom.OnEventStateChanged` and retakes when `IsPreFinished` flips false → true across the call — that is the call that marks the room and fires `SaveRun(this)`, after the chosen option's `RelicCmd.Obtain` and `UpdateRunHistory` have already run. Ordinary events return early and never flip it; a room loaded pre-finished starts true and never retakes. The method is only reached through the `StateChanged` delegate, so no caller can have inlined past the patch.
  - A replayed pre-finished `CombatRoom` goes through `StartPreFinishedCombat`, which fires neither `AfterRoomEntered` nor `CombatEnded`, so the retaken snapshot survives the restart and a second restart rewinds to it again. A replayed Ancient does fire `AfterRoomEntered`, which recaptures the just-rewound record — the same content.
- `RunTracker.RollBackToRoomEntry` restores whichever snapshot is current, and `RoomResetter` calls it after every abort path and immediately before `CleanUp()` — the point of no return — so a corrupt-save abort leaves both the game and the record untouched. It writes via `RunStorage.SaveAsync` rather than `SaveCurrentRun`, because the latter re-stamps the still-live (post-purchase) instance numbers over the restored ones. The live identity maps are left alone on purpose: `RunStarted` re-fires moments later and `AdoptRunLocked` clears and rebuilds them from the restored record against the reloaded deck.

The snapshot is held in memory and mirrored to disk at every capture, under `user://SpireLens/room-entry/<run_id>.json` — deliberately not under `runs/`, which is enumerated as run records. A Core hot reload mid-room therefore keeps the restart available: `EnsureRoomEntrySnapshotLoadedLocked` reads the file back when memory is empty. A capture that fails deletes the file instead of leaving the previous snapshot behind, because an older snapshot read back after a reload no longer matches the save. With no snapshot at all, `Describe()` blocks with `no room-entry snapshot` — refusing a restart is cheap, silently inflating or erasing part of a run is not.
